...
class HostelMaintenanceScreen extends StatefulWidget {
  ...
  @override
  State<HostelMaintenanceScreen> createState() =>
      _HostelMaintenanceScreenState();
}
...
class _HostelMaintenanceScreenState extends State<HostelMaintenanceScreen> {
  ...
  Future<void> _loadRequests() async {
    try {
      final String response = await rootBundle.loadString(
        'assets/mock/hostel_maintenance.json',
      );
  ...
  Widget build(BuildContext context) {
    ...
    final displayed = _requests.where((item) {
      ...
      bool matchesBlock =
          _selectedBlock == 'All Blocks' ||
          blockStr.contains(_selectedBlock.split(' ').last);
  ...
  Widget _buildRequestCard(Map<String, dynamic> req) {
    ...
    switch (priority) {
      case 'High':
        ...
        priColor = const Color(0xFFE11D48);
        break;
      case 'Medium':
        priBg = const Color(0xFFFEF3C7);
        priColor = const Color(0xFFD97706);
        break;
      case 'Low':
        priBg = const Color(0xFFDCFCE7);
        priColor = const Color(0xFF16A34A);
        break;
      default:
        priBg = const Color(0xFFF1F5F9);
        priColor = const Color(0xFF64748B);
    }
              ...
              children: [
                ...
                PopupMenuButton<String>(
                  ...
                  onSelected: (val) {
                    ...
                    if (val == 'progress') {
                      setState(() => req['status'] = 'In Progress');
                    }
                    if (val == 'resolve') {
                      setState(() => req['status'] = 'Resolved');
                    }
                    if (val == 'delete') {
                      setState(
                        () =>
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildSheetDropdown(
                    'Block',
                    _selectedBlock,
                    ['All Blocks', 'Block A', 'Block B', 'Block C'],
                    (val) {
                      if (val != null) {
                        setModalState(() => _selectedBlock = val);
                        setState(() {});
                      }
                    },
                  ),
              ...
              children: [
                    ...
                    const Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 4),
                          Text(
                            'Update the details of the maintenance issue.',
                            style: TextStyle(
                              fontSize: 13,
                              color: Color(0xFF64748B),
                            ),
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 14),
                    Expanded(
                      child: _buildField('Reported By', reportedByCtrl, ''),
                    ),
                  ],
                ),
                const SizedBox(height: 16),
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        final title =
                            '$category Issue in ${roomCtrl.text.trim().isNotEmpty ? roomCtrl.text.trim() : 'Room'}';
                        final initials = category.substring(0, 2).toUpperCase();
                        setState(() {
                          req['title'] = title;
                          req['category'] = category;
                          req['description'] = descCtrl.text.trim().isNotEmpty
                              ? descCtrl.text.trim()
                              : 'No description';
                          req['roomNo'] = roomCtrl.text.trim().isNotEmpty
                              ? roomCtrl.text.trim()
                              : 'N/A';
                          req['block'] = block;
                          req['reportedBy'] =
                              reportedByCtrl.text.trim().isNotEmpty
                              ? reportedByCtrl.text.trim()
                              : 'Staff';
                        ...
                        setState(() {
                          _requests.insert(0, {
                            ...
                            'description': descCtrl.text.trim().isNotEmpty
                                ? descCtrl.text.trim()
                                : 'No description',
                            'roomNo': roomCtrl.text.trim().isNotEmpty
                                ? roomCtrl.text.trim()
                                : 'N/A',
                            'block': block,
                            'reportedBy': reportedByCtrl.text.trim().isNotEmpty
                                ? reportedByCtrl.text.trim()
                                : 'Staff',
                            'rollNo': 'N/A',
