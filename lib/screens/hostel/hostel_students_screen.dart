...
class _HostelStudentsScreenState extends State<HostelStudentsScreen> {
  ...
  Future<void> _loadStudents() async {
    try {
      final String response = await rootBundle.loadString(
        'assets/mock/hostel_students.json',
      );
  ...
  Widget build(BuildContext context) {
    ...
    int newAdmissionsCount =
        10 +
        _students
            .where(
              (s) =>
                  s['joinedDate'] == 'Today' ||
                  s['joinedDate'] == '20 Jun 2025',
            )
            .length;

// ignore: unused_local_variable
    bool isCustomFilterActive =
        _filterBlock != 'All Blocks' ||
    ...
    List<Map<String, dynamic>> displayedStudents = _students.where((s) {
      ...
      bool matchesBlock =
          _filterBlock == 'All Blocks' ||
          block.contains(_filterBlock.split(' ')[0].toLowerCase());
      bool matchesRoom =
          _filterRoom == 'All Rooms' ||
          roomNo == _filterRoom.replaceAll('Room ', '').toLowerCase();
      bool matchesClass =
          _filterClass == 'All Classes' ||
          grade.contains(_filterClass.toLowerCase());
  ...
  Widget _buildStudentCard(Map<String, dynamic> student) {
    ...
    final isCheckedIn =
        status == 'Checked In' || status == 'Allocated' || status == 'Active';
  ...
  void _showViewDetailsModal(Map<String, dynamic> student) {
    ...
    final isCheckedIn =
        status == 'Checked In' || status == 'Allocated' || status == 'Active';
    final isPending = status == 'Pending';
    final displayStatus = isCheckedIn
        ? 'Active'
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 2),
                          Text(
                            'Roll No: ${student['rollNo'] ?? 'HST-101'} • ${student['admNo']} • ${student['grade']}',
                            style: const TextStyle(
                              fontSize: 13,
                              fontWeight: FontWeight.w500,
                              color: Color(0xFF6C6C80),
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: _buildGridDetailItem(
                            'ROLL NUMBER',
                            '${student['rollNo'] ?? 'HST-101'}',
                          ),
                        ),
                        Expanded(
                          child: _buildGridDetailItem(
                            'CLASS & SECTION',
                            '${student['grade'] ?? 'Grade 9-A'}',
                          ),
                        ),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: _buildGridDetailItem(
                            'GENDER & AGE',
                            '${student['type'] ?? 'Boys'} • ${student['age'] ?? '15'} Yrs',
                          ),
                        ),
                        Expanded(
                          child: _buildGridDetailItem(
                            'ADMISSION NO.',
                            '${student['admNo'] ?? 'ADM2024-101'}',
                          ),
                        ),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: _buildGridDetailItem(
                            'ROOM NO.',
                            '${student['roomNo']} (${student['bedNo'] ?? 'Bed A'})',
                          ),
                        ),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: _buildGridDetailItem('STATUS', displayStatus),
                        ),
                        Expanded(
                          child: _buildGridDetailItem(
                            'CHECK IN DATE',
                            '${student['joinedDate'] ?? '12 Jun 2025'}',
                          ),
                        ),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: _buildGridDetailItem(
                            'GUARDIAN NAME',
                            '${student['guardianName'] ?? 'Ramesh Mehta'}',
                          ),
                        ),
                        Expanded(
                          child: _buildGridDetailItem(
                            'CONTACT NUMBER',
                            '${student['contact'] ?? '+91 98765 43210'}',
                          ),
                        ),
                      ],
                    ),
  ...
  void _showEditStudentModal(Map<String, dynamic> student) {
    ...
    if (!['Boys', 'Girls', 'Male', 'Female'].contains(selectedGender)) {
      selectedGender = 'Boys';
    }
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    'Modify information for ${student['student']}.',
                    style: const TextStyle(
                      fontSize: 13,
                      color: Color(0xFF6C6C80),
                    ),
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () {
                          ...
                          final newGuardian = guardianNameController.text
                              .trim();
                          final newContact = contactController.text.trim();

                          final nameParts = newName.split(' ');
                          final newInitials = nameParts.length > 1
                              ? '${nameParts[0][0]}${nameParts[1][0]}'
                                    .toUpperCase()
                              : newName
                                    .substring(0, newName.length >= 2 ? 2 : 1)
                                    .toUpperCase();
                          ...
                          setState(() {
                            if (newName.isNotEmpty) {
                              student['student'] = newName;
                            }
                            if (newInitials.isNotEmpty) {
                              student['initials'] = newInitials;
                            }
                            ...
                            if (newGuardian.isNotEmpty) {
                              student['guardianName'] = newGuardian;
                            }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'Are you sure you want to delete ${student['student']} (${student['rollNo']}) from hostel records? This action cannot be undone.',
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  const Text(
                    'Register a new student into the hostel system. Complete all mandatory fields below.',
                            ...
                            Container(
                              ...
                              child: DropdownButtonHideUnderline(
                                child: DropdownButton<String>(
                                  ...
                                  onChanged: (val) {
                                    if (val != null) {
                                      setModalState(() => selectedBlock = val);
                                    }
                                  },
                                ),
                              ),
                      ...
                      ElevatedButton(
                        onPressed: () {
                          ...
                          final guardian =
                              ...
                              : 'Ramesh Mehta';
                          final phone = contactController.text.trim().isNotEmpty
                          ...
                          setState(() {
                            _students.insert(0, {
                              ...
                              'rollNo': roll,
                              'admNo':
                                  'ADM2024-${roll.replaceAll(RegExp(r'[^0-9]'), '')}',
                              'grade': grade,
                              'block': block,
                              'blockCode': block.contains('(')
                                  ? block.split('(')[1].replaceAll(')', '')
                                  : 'A',
                          ...
                          scaffoldMessenger.showSnackBar(
                            SnackBar(
                              content: Text(
                                '$name added to Hostel Students successfully!',
                              ),
                            ),
                          );
                        },
