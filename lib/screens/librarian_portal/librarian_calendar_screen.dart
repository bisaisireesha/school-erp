...
Color _librarianTypeColor(String? t) {
  switch (t) {
    case 'Book Fair':
    case 'Exhibition':   return const Color(0xFF6C4CF1);
    case 'Stock Audit':
    case 'Maintenance':  return const Color(0xFFF59E0B);
    case 'Orientation':
    case 'Meeting':      return const Color(0xFF3B82F6);
    case 'Author Visit':
    case 'Workshop':     return const Color(0xFF10B981);
...
// ─── Librarian Calendar Screen (Cloned from Driver / Transport Pattern) ───────

class LibrarianCalendarScreen extends StatefulWidget {
  final Map<String, dynamic> data;
  final VoidCallback? onBack;

  const LibrarianCalendarScreen({
    super.key,
    required this.data,
    this.onBack,
  });
  ...
  @override
  State<LibrarianCalendarScreen> createState() => _LibrarianCalendarScreenState();
}

class _LibrarianCalendarScreenState extends State<LibrarianCalendarScreen> {
  ...
  void _loadEvents() {
    final raw = widget.data['calendarEvents'] as List? ?? widget.data['events'] as List? ?? [];
    final now = DateTime.now();

    _allEvents = [
      ...raw.map((e) => Map<String, dynamic>.from(e as Map)),
      {
        'title': 'Annual School Book Fair & Exhibition',
        'date': _fmtDate(now.add(const Duration(days: 2))),
        'time': '09:00 AM - 04:30 PM',
        'location': 'Central Library Hall',
        'type': 'Book Fair',
        'notes': 'Publishers setup, student book browsing sessions, and author corner.',
      },
      {
        'title': 'New Student Library Orientation Session',
        'date': _fmtDate(now.add(const Duration(days: 4))),
        'time': '10:30 AM - 12:00 PM',
        'location': 'Reading Room 1',
        'type': 'Orientation',
        'notes': 'Demonstration of OPAC catalog, digital library access, and borrowing rules.',
      },
      {
        'title': 'Quarterly Book Inventory & Rack Audit',
        'date': _fmtDate(now.add(const Duration(days: 8))),
        'time': '02:00 PM - 05:00 PM',
        'location': 'Stacks & Shelves Section',
        'type': 'Stock Audit',
        'notes': 'Barcode reconciliation for Science, Fiction, and Reference sections.',
      },
      {
        'title': 'Guest Author Reading: Dr. Priya Nair',
        'date': _fmtDate(now.add(const Duration(days: 12))),
        'time': '11:00 AM - 01:00 PM',
        'location': 'Library Audio-Visual Room',
        'type': 'Author Visit',
        'notes': 'Interactive session with Middle and High School literature students.',
      },
      {
        'title': 'Independence Day Library Closure',
        'date': _fmtDate(now.add(const Duration(days: 14))),
        'time': 'Full Day',
        'location': 'School Campus',
        'type': 'Holiday',
        'notes': 'Library services closed for National Holiday.',
      },
    ];
  ...
  void _showEventDetailsBottomSheet(Map<String, dynamic> event) {
    ...
    final String type = event['type'] as String? ?? 'Book Fair';
    final String notes = event['notes'] as String? ??
        'Please verify book stock, issue counters, and catalog records prior to this event.';
    final Color color = _librarianTypeColor(type);
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        _buildDetailRow(LucideIcons.clock, 'Time', time.isNotEmpty ? time : 'All Day'),
                        if (location.isNotEmpty) ...[
                          const Divider(height: 1, color: Color(0xFFF0EDF8), indent: 14, endIndent: 14),
                          _buildDetailRow(LucideIcons.mapPin, 'Location', location),
                        ],
                        const Divider(height: 1, color: Color(0xFFF0EDF8), indent: 14, endIndent: 14),
                        _buildDetailRow(LucideIcons.tag, 'Category', type),
                      ],
                    ),
                  ),
                  const SizedBox(height: 16),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        leading: widget.onBack != null ? AppBackButton(onPressed: widget.onBack!) : null,
        title: const Text(
          'Library Calendar',
          style: TextStyle(
            color: Color(0xFF1E1E2D),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Expanded(
                  child: Text(
                    'Library Schedule (${_shortMonths[_focusedMonth.month - 1]} ${_focusedMonth.year})',
                    style: const TextStyle(
                      fontSize: 16.0,
                      fontWeight: FontWeight.bold,
                      color: Color(0xFF1E1E2D),
          ...
          child: Row(
            children: row.map((day) {
              ...
              final Color dotColor = hasEvent
                  ? _librarianTypeColor(_eventsByKey[key]!.first['type'] as String?)
                  : Colors.transparent;
  ...
  Widget _buildEventCard(Map<String, dynamic> e) {
    ...
    final String type = e['type'] as String? ?? 'Book Fair';
    final Color color = _librarianTypeColor(type);
  ...
  Widget _buildEmptyMonthCard() {
    return Container(
      ...
      child: const Center(
        child: Text(
          'No library events scheduled this month.',
          style: TextStyle(
            fontSize: 13.0,
