          ...
          children: [
            ...
            _buildSectionTitle(
              ...
              onTap: () => MainLayout.pushSubScreen(
                context,
                AccountantPaymentHistoryScreen(
                        ...
                        Container(
                          ...
                          child: Text(
                            ...
                            style: TextStyle(
                              fontSize: 10,
                              fontWeight: FontWeight.bold,
                              color: txn['statusColor'] as Color,
                            ),
                          ),
                        ),
                      ],
                ...
                children: [
                  GestureDetector(
                    onTapUp: (details) {
                      ...
                      if (dx > leftPadding) {
                        final index = ((dx - leftPadding) / barGroupWidth)
                            .floor();
                  ...
                  if (_selectedChartIndex != null)
                    Positioned(
                      ...
                      left: math.max(
                        ...
                        math.min(
                          chartWidth -
                              100, // Tooltip roughly 100px wide, prevent clipping on right
                          30.0 +
                              ((chartWidth - 30.0) / months.length) *
                                  _selectedChartIndex! +
                              ((chartWidth - 30.0) / months.length) / 2 -
                              50,
                        ),
                      ),
                      child: Container(
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: (mode['color'] as Color).withValues(
                            alpha: 0.1,
                          ),
                          borderRadius: BorderRadius.circular(6),
                        ),
              ...
              child: OutlinedButton.icon(
                onPressed: () {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Reminders sent to all overdue accounts!'),
                    ),
                  );
                },
            ...
            children: actions.map((action) {
              return GestureDetector(
                onTap: () {
                  ...
                  Widget? screen;
                  if (key == 'fee') {
                    screen = AccountantQuickCollectionScreen(
                      onBack: () => MainLayout.popSubScreen(context),
                    );
                  } else if (key == 'expense') {
                    screen = AccountantExpensesScreen(
                      onBack: () => MainLayout.popSubScreen(context),
                    );
                  } else if (key == 'invoice') {
                    screen = AccountantInvoicesScreen(
                      onBack: () => MainLayout.popSubScreen(context),
                    );
                  } else if (key == 'reports') {
                    screen = AccountantReportsScreen(
                      onBack: () => MainLayout.popSubScreen(context),
                    );
                  } else if (key == 'payroll') {
                    screen = AccountantPaySlipsScreen(
                      onBack: () => MainLayout.popSubScreen(context),
                    );
                  } else if (key == 'bank') {
                    screen = AccountantBankAccountsScreen(
                      onBack: () => MainLayout.popSubScreen(context),
                    );
                  }
...
class _BarChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= gridCount; i++) {
      ...
      // Y-axis label
      final label =
          '${((gridCount - i) / gridCount * maxValue).toStringAsFixed(0)}L';
    ...
    for (int i = 0; i < months.length; i++) {
      ...
      monthTp.paint(
        canvas,
        Offset(
          groupCenterX - monthTp.width / 2,
          topPadding + chartAreaHeight + 6,
        ),
      );
    }
  }
