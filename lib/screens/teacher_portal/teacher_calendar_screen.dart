...
Color _teacherTypeColor(String? t) {
  switch (t) {
    case 'Academic':  return const Color(0xFF6C4CF1);
    case 'Exam':
    case 'Exams':     return const Color(0xFFF59E0B);
    case 'Meeting':
    case 'PTA':       return const Color(0xFF3B82F6);
    case 'Workshop':
...
// ─── Teacher Calendar Screen (Cloned from Driver / Transport Pattern) ─────────
...
class _TeacherCalendarScreenState extends State<TeacherCalendarScreen> {
  ...
  void _loadEvents() {
    ...
    _allEvents = [
      ...raw.map((e) => Map<String, dynamic>.from(e as Map)),
      {
        ...
        'location': 'Hall 3, Block A',
        'type': 'Exam',
        'notes': 'Invigilation duty assigned. Collect test booklets from Exam Cell 15 mins prior.',
      },
      {
        'title': 'Faculty & Department Review Meeting',
        'date': _fmtDate(now.add(const Duration(days: 3))),
        'time': '02:30 PM - 03:45 PM',
        'location': 'Conference Room B',
        'type': 'Meeting',
        'notes': 'Discussion on Term 1 syllabus coverage and upcoming formative assessments.',
      },
      {
        'title': 'Science & Mathematics Exhibition',
        'date': _fmtDate(now.add(const Duration(days: 7))),
        'time': '10:00 AM - 04:00 PM',
        'location': 'School Auditorium',
        'type': 'Academic',
        'notes': 'Guide student projects for Class 10-A and 9-C submissions.',
      },
      {
        'title': 'Teacher Professional Development Workshop',
        'date': _fmtDate(now.add(const Duration(days: 11))),
        'time': '09:30 AM - 01:00 PM',
        'location': 'AV Room 2',
        'type': 'Workshop',
        'notes': 'Modern pedagogical strategies and interactive STEM teaching modules.',
      },
      {
        ...
        'type': 'Holiday',
        'notes': 'Flag hoisting ceremony followed by school holiday.',
      },
    ];
  ...
  void _showEventDetailsBottomSheet(Map<String, dynamic> event) {
    ...
    final String type = event['type'] as String? ?? 'Academic';
    final String notes = event['notes'] as String? ??
        'Please ensure all academic materials, rosters, and teaching guidelines are prepared for this event.';
    final Color color = _teacherTypeColor(type);
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Expanded(
                  child: Text(
                    'Events & Schedule (${_shortMonths[_focusedMonth.month - 1]} ${_focusedMonth.year})',
                    style: const TextStyle(
                      fontSize: 16.0,
                      fontWeight: FontWeight.bold,
                      color: Color(0xFF1E1E2D),
          ...
          child: Row(
            children: row.map((day) {
              ...
              final Color dotColor = hasEvent
                  ? _teacherTypeColor(_eventsByKey[key]!.first['type'] as String?)
                  : Colors.transparent;
  ...
  Widget _buildEventCard(Map<String, dynamic> e) {
    ...
    final String type = e['type'] as String? ?? 'Academic';
    final Color color = _teacherTypeColor(type);
