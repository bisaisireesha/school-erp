...
class _VendorsScreenState extends State<VendorsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredVendors = _vendors.where((vendor) {
      ...
      if (_selectedFilter == 'Active' || _selectedFilter == 'On Hold') {
        return vendor['status'].toString().toLowerCase() == _selectedFilter.toLowerCase();
      }
                ...
                child: Column(
                  children: [
                    if (filteredVendors.isEmpty)
                      ...
                      ...filteredVendors.map((vendor) => _buildVendorItem(vendor)),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(LucideIcons.store, color: Color(0xFF64748B), size: 32),
          ),
          const SizedBox(height: 16),
          const Text(
            'No vendors found',
  ...
  Widget _buildKPIs() {
    ...
    final onHoldVendors = _vendors.where((v) => v['status'] == 'On Hold').length;
    
    int totalDues = 0;
    for (var v in _vendors) {
      final duesStr = (v['dues'] ?? '0').toString().replaceAll(',', '').replaceAll('₹', '').trim();
      totalDues += int.tryParse(duesStr) ?? 0;
    }
    final duesFormatted = '₹${totalDues.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Row(
            children: [
              Expanded(child: _buildKpiCard('Total Vendors', '$totalVendors', LucideIcons.briefcase, const Color(0xFF6C4CF1), const Color(0xFFF3F0FF))),
              const SizedBox(width: 12),
              Expanded(child: _buildKpiCard('Active', '$activeVendors', LucideIcons.checkCircle, const Color(0xFF10B981), const Color(0xFFDCFCE7))),
            ],
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(child: _buildKpiCard('On Hold', '$onHoldVendors', LucideIcons.alertCircle, const Color(0xFFF59E0B), const Color(0xFFFEF3C7))),
              const SizedBox(width: 12),
              Expanded(child: _buildKpiCard('Outstanding', duesFormatted, LucideIcons.wallet, const Color(0xFFEF4444), const Color(0xFFFEE2E2))),
            ],
          ),
        ],
      ),
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  const Text('Filter Vendors', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E1E2D))),
                  GestureDetector(
  ...
  Widget _buildKpiCard(String label, String count, IconData icon, Color textColor, Color bgColor) {
    return Container(
      ...
      child: Row(
        children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: bgColor, borderRadius: BorderRadius.circular(16)), child: Icon(icon, color: textColor, size: 24)),
          const SizedBox(width: 16),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(count, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF1E1E2D))),
          ...
          child: Row(
            ...
            children: [
              Row(
                children: [
                  ...
                  const Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Vendors', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Color(0xFF1E1E2D))),
                      Text('Manage hostel suppliers', style: TextStyle(fontSize: 13, color: Color(0xFF64748B), fontWeight: FontWeight.w500)),
                    ],
                  ),
                ],
              ),
              ElevatedButton.icon(
                ...
                label: const Text('Add Vendor', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.white)),
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF6C4CF1),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.store, color: Color(0xFF6C4CF1), size: 20),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(vendor['name'] as String, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E1E2D)), maxLines: 1, overflow: TextOverflow.ellipsis),
                    Text(vendor['category'] as String, style: const TextStyle(fontSize: 13, color: Color(0xFF6C6C80), fontWeight: FontWeight.w500)),
                  ],
                ),
              ),
              Container(
              ...
              PopupMenuButton<String>(
                ...
                onSelected: (value) {
                  if (value == 'view') {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Details sent to accountant for ${vendor['name']}')));
                  } else if (value == 'edit') {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${vendor['name']} removed from vendors')));
                  }
                },
                itemBuilder: (context) => [
                  _buildPopupItem('view', LucideIcons.eye, 'View Details'),
                  _buildPopupItem('send', LucideIcons.send, 'Send to Accountant'),
                  _buildPopupItem('edit', LucideIcons.edit2, 'Edit Vendor'),
                  _buildPopupItem('remove', LucideIcons.trash2, 'Remove Vendor', isDestructive: true),
                ],
              ),
            ],
          ),
          ...
          Row(
            children: [
              Expanded(child: _buildDetailItem('Contact Person', vendor['contactPerson'] as String)),
              Expanded(child: _buildDetailItem('Phone', vendor['phone'] as String)),
            ],
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(child: _buildDetailItem('Email', vendor['email'] as String)),
  ...
  void _showAddVendorBottomSheet(BuildContext context, {Map<String, dynamic>? vendor}) {
    final isEditing = vendor != null;
    String selectedCategory = isEditing ? vendor['category'] as String : 'Grains';
    ...
    final contactPersonController = TextEditingController(text: isEditing ? vendor['contactPerson'] : '');
    final phoneController = TextEditingController(text: isEditing ? vendor['phone'] : '');
    final emailController = TextEditingController(text: isEditing ? vendor['email'] : '');
    final duesController = TextEditingController(text: isEditing ? vendor['dues'] : '');
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text(isEditing ? 'Edit Vendor' : 'Add Vendor', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1E1E2D))),
                          GestureDetector(
                            onTap: () => Navigator.pop(context),
                      ...
                      _buildDialogTextField('E.g., Sri Annapurna Traders', controller: nameController),
                      const SizedBox(height: 16),
                      Row(
                        children: [
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                _buildDialogDropdown(
                                  ...
                                  (val) => setSheetState(() => selectedCategory = val!),
                                ),
                              ],
                            ),
                          ),
                      ...
                      _buildDialogTextField('E.g., Mr. Suresh K.', controller: contactPersonController),
                      const SizedBox(height: 16),
                      Row(
                        children: [
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                _buildDialogTextField('+91 ...', controller: phoneController),
                              ],
                            ),
                          ),
                          const SizedBox(width: 16),
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                _buildDialogTextField('email@domain.com', controller: emailController),
                              ],
                            ),
                          ),
                        ],
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          ElevatedButton(
                            onPressed: () {
                              final newVendor = {
                                'id': isEditing ? vendor['id'] : DateTime.now().millisecondsSinceEpoch.toString(),
                                'name': nameController.text.isNotEmpty ? nameController.text : 'New Vendor',
                                'category': selectedCategory,
                                'status': selectedStatus,
                                'statusColor': selectedStatus == 'Active' ? const Color(0xFF16A34A) : const Color(0xFFD97706),
                                'statusBg': selectedStatus == 'Active' ? const Color(0xFFDCFCE7) : const Color(0xFFFEF3C7),
                                'contactPerson': contactPersonController.text,
                                'phone': phoneController.text,
                                'email': emailController.text,
                                'dues': duesController.text.isNotEmpty ? duesController.text : '0',
                              };
                              setState(() {
                                if (isEditing) {
                            ...
                            child: Text(isEditing ? 'Save Changes' : 'Add Vendor', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                          ),
                        ],
                      ),
                    ],
  ...
  void _showViewDetailsBottomSheet(BuildContext context, Map<String, dynamic> vendor) {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      builder: (context) {
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('${vendor['name']} Details', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1E1E2D))),
                  GestureDetector(
                    onTap: () => Navigator.pop(context),
                    child: const Icon(LucideIcons.x, color: Color(0xFF64748B), size: 24),
                  ),
              ...
              Row(
                children: [
                  Container(
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(color: const Color(0xFFF3F0FF), borderRadius: BorderRadius.circular(12)),
                    child: const Icon(LucideIcons.store, color: Color(0xFF6C4CF1), size: 28),
                  ),
                  const SizedBox(width: 16),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(vendor['name'] as String, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E1E2D))),
                        Text(vendor['category'] as String, style: const TextStyle(fontSize: 14, color: Color(0xFF6C6C80), fontWeight: FontWeight.w500)),
                      ],
                    ),
                  ),
                  Container(
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  _buildDetailItem('Contact Person', vendor['contactPerson'] as String),
  ...
  Widget _buildDialogDropdown(String value, List<String> options, ValueChanged<String?> onChanged) {
    return Container(
      height: 50,
      padding: const EdgeInsets.symmetric(horizontal: 16),
      decoration: BoxDecoration(
